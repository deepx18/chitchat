...
export default function Sidebar() {
  ...
  useEffect(() => {
    // ApiHandler.getChats(currentUser.user_id, setChats);
  }, []);
  ...
  return (
    <aside className="h-screen w-80 fixed left-0 top-0 bg-[#f9f2f2] dark:bg-stone-900 flex flex-col py-8 px-6 space-y-6 z-20">
      ...
      <button className="w-full bg-primary py-3 rounded-lg text-on-primary font-manrope font-bold text-sm hover:bg-primary-dim transition-all active:scale-95">
        New Conversation
      </button>
      <nav className="flex-1 overflow-y-auto space-y-1 mt-4">
        ...
        {chats?.map( (chat, i) => {
          const idx = chat.users.indexOf(currentUser.user_id) === 0 ? 1 : 0;          
          const user_id = chat.users[idx];

          const user = ApiHandler.getUser(user_id)

          

          return (
            <a
              className="flex flex-col space-y-1 px-4 py-4 rounded bg-[#efe6e7] border-r-2 border-[#0033dc]"
              href="#"
              key={i}
            >
